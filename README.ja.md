...
| 9 | トリガー: 曜日 × 時間帯スケジュール | [#63](https://github.com/ryouka0731/KeepingYouAwake/pull/63) |
| 10 | トリガー: ダウンロード進行中 (`*.crdownload` 等) | [#64](https://github.com/ryouka0731/KeepingYouAwake/pull/64) |
| 11 | メニューバー残り時間カウントダウン表示 | [#55](https://github.com/ryouka0731/KeepingYouAwake/pull/55) |
| 12 | アクティビティログ (`~/Library/Application Support/KeepingYouAwake/activity.jsonl`) | [#59](https://github.com/ryouka0731/KeepingYouAwake/pull/59) / [#61](https://github.com/ryouka0731/KeepingYouAwake/pull/61) |

加えて session source tracking を導入し、機能トリガーがユーザー手動セッションを誤って終了させない設計に refactor 済 (外部ディスプレイ trigger も [#58](https://github.com/ryouka0731/KeepingYouAwake/pull/58) で source-aware 化)。Sparkle auto-update の appcast 自動生成 workflow も用意済 ([#66](https://github.com/ryouka0731/KeepingYouAwake/pull/66)、最終手順は `docs/sparkle-auto-update-setup.md`)。
...
# スケジュール trigger (windows は PlistBuddy で別途定義)
defaults write info.marcel-dierkes.KeepingYouAwake \
...
# メニューバー countdown を非表示 (デフォルト: 表示)
defaults write info.marcel-dierkes.KeepingYouAwake \
